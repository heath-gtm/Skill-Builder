...
When this skill is invoked inside the Monthly workflow, add the following dimensions to every analysis:

### 1. Month-over-Month (MoM) delta

For each top-line metric (Total ARR, DS, SS, Net ARR Contribution, New Business, Expansion, Downgrade, Churn, Reactivation):
- Read current month value
- Read prior month value (from prior monthly snapshot at `~/Library/Mobile Documents/com~apple~CloudDocs/Anthropic/Revenue Reviews/Monthly Snapshots/Mixmax_Monthly_Snapshot_{YYYY-MM-PRIOR}.xlsx`, if available)
- Compute absolute $ delta and % delta
- Narrate direction (improved / degraded / flat) and magnitude

If the prior-month snapshot is unavailable, note "Prior-month comparison unavailable — first run of monthly cadence" and proceed without MoM.

### 2. Quarter-to-Date (QTD) rollup

For each Net ARR Contribution line:
- Sum the closed months in the current quarter (e.g., if closing March, sum Jan + Feb + Mar)
- Compare QTD actual vs QTD target
- Compute variance $ and %
- This is the field that rolls into the quarterly report — accuracy here is non-negotiable

QTD sums can be pulled from the Quarterly Revenue Summary tab (GID 2066645768) — read those cells, do not recompute from monthly totals.

### 3. Driver attribution

For any material variance vs target (>5% or >$25K), attribute to exactly one of:
- New Business under/over
- Expansion under/over
- Reactivation under/over
- Upgrade under/over
- Downgrade under/over (budget)
- Churn under/over (budget)
- Self-Serve net motion

Call out the top 1–2 drivers in narrative. Do not over-explain.

### 4. Forecast accuracy (closed period)

At the top of the revenue section, include a forecast-accuracy callout:
> Month-start Commit forecast: $X
> Month-end Actual: $Y
> Accuracy: Z% (within target band / outside band)

A band of ±5% is acceptable. Outside that band, the variance goes into the risk register as a Forecast Miss.

### 5. Output must be reconciliation-ready

Every number you produce must cite its source cell AND be ready for comparison to the final weekly report for the month. The accuracy-reconciliation skill will run these numbers through the gate — if they can't be traced, they don't publish.
