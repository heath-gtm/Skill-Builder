...
In the Monthly workflow, renewals analysis is deep AND forward-looking.

### Retrospective — the closed month

For the month just closed, report:
...
| # Churned | X | — | — |
| $ Churn impact | −$X | −$Y | ±$Z |
| $ Downgrade impact | −$X | −$Y | ±$Z |
| NRR % (actual) | X% | Y% | ±Zpp |
| GRR % (ARR retention) | X% | Y% | ±Zpp |

For every churned or downgraded account:
- Account name (redact for GTM report; full name for CRO report)
- ARR amount
- Reason (competitive, product gap, budget, consolidation, M&A, no contact, other)
- CSM owner (CRO report only)

### Forward-looking — at-risk book

For the new month (and the remainder of the quarter), report:
- # accounts At Risk
- Total At Risk ARR
- Break down by segment: CSM Owned vs VSB Owned
- Top 5 at-risk accounts by $ exposure, each with:
  - Renewal date
  - CSM owner
  - Current risk level
  - Specific next-step action before renewal

Every at-risk account with >$25K ARR exposure goes into the risk register as a CS At-Risk item with Month Impact $0 and QTD Impact = exposure.

### Reason taxonomy (standardize for reporting)

Use exactly these churn reasons (no free-text):
- Competitive loss (name competitor in CRO report)
- Product gap (specify feature)
- Budget / pricing
- Consolidation (customer consolidating vendors)
- M&A (customer acquired / merged)
- No contact (CSM unable to reach)
- Sponsor change
- Low usage / low engagement
- Other

This enables period-over-period trend analysis. Free-text reasons make MoM analysis impossible.
