...
```bash
mkdir -p ~/.claude/skills/decision-log && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/decision-log/SKILL.md -o ~/.claude/skills/decision-log/SKILL.md && echo "Installed decision-log. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/decision-log/`, then restart Claude Code.
