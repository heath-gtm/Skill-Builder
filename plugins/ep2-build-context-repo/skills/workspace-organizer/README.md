...
> Make a messy repo, folder, or workspace navigable, without moving a thing until you approve. Propose a naming convention, a folder structure, an index or README, and a cleanup plan, shown as a plan first so you can see every move before anything happens. Trigger on "organize this folder", "this repo is a mess", "clean up my workspace", "come up with a naming convention", "where should this live", or any pile of files you can no longer find things in.
...
```bash
mkdir -p ~/.claude/skills/workspace-organizer && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/workspace-organizer/SKILL.md -o ~/.claude/skills/workspace-organizer/SKILL.md && echo "Installed workspace-organizer. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/workspace-organizer/`, then restart Claude Code.
