...
```bash
mkdir -p ~/.claude/skills/demand-gen-plan && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/demand-gen-plan/SKILL.md -o ~/.claude/skills/demand-gen-plan/SKILL.md && echo "Installed demand-gen-plan. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/demand-gen-plan/`, then restart Claude Code.
