...
```bash
mkdir -p ~/.claude/skills/comp-quota-plan && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/comp-quota-plan/SKILL.md -o ~/.claude/skills/comp-quota-plan/SKILL.md && echo "Installed comp-quota-plan. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/comp-quota-plan/`, then restart Claude Code.
