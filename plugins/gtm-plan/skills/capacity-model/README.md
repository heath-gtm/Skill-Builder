...
```bash
mkdir -p ~/.claude/skills/capacity-model && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/capacity-model/SKILL.md -o ~/.claude/skills/capacity-model/SKILL.md && echo "Installed capacity-model. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/capacity-model/`, then restart Claude Code.
