---
name: direct-sales
description: Business-unit lens on Mixmax Direct Sales performance for the Quarterly report. Action-forward — every section produces named next steps with owner, dollar impact, and research-backed context. Use whenever the quarterly analysis needs rep-level attainment, deal-by-deal forecast quality, unstick-the-stuck deal plays, new-business target identification, segment mix, sales velocity, or early-churn attribution on DS-owned deals. Trigger on phrases like "Direct Sales", "DS engine", "rep performance", "rep attainment", "AE quota", "deal autopsy", "deals to move", "deals to unstick", "new business targets", "sales velocity", "segment mix", "SMB/MM/Enterprise", "pipeline concentration", "forecast accuracy by rep", "early churn", "90-day churn", "ramp health", or any request for a DS-specific read.
---

# Direct Sales — Quarterly Specialist Lens

The DS engine autopsy. Every output ends in named actions with owners and dollar impact. No section is allowed to be "here are the numbers" without "here's what to do about it."
...
### 3. Top Deals to Move (NEXT 60 DAYS) — THE HEART

This is the section the CRO / AEs will read first. Rank order the **top 10 deals** that need attention next 60 days. Include:
- **Unstick plays** — deals stuck >14 days in stage, with specific identified blocker
- **Pull-in candidates** — Best Case deals with momentum that could convert to Commit
- **Expansion conversions** — Expansion Commit deals with multi-product potential
- **New-logo race** — deals competing with a known alternative, where speed wins

For each deal, follow the Action Item Format exactly.

### 4. New-Business Targets for Next Quarter

Top 10 accounts to prospect next quarter. Pull from Pipeline Building's Top 10 but **DS-filter**: only accounts that fit DS (not Product-led), with Aero Account Fit ≥ Good, and where the buyer is a known DS pattern (sales leader, ops leader buying for a team).

**Research each one.** `enrich_company` for firmographic + tech stack, `WebSearch` for recent news, `generate_call_prep` if Aero agent is connected. Then write a 2-sentence WHY NOW grounded in the research, not generic.
