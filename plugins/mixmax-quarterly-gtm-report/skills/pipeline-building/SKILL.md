...
In the Monthly workflow, pipeline-building produces both a retrospective and a forward-looking view.

### Retrospective (included in closed-month report)

For each channel (Inbound, Outbound, Product, Expansion):
- New pipeline created this month (actual vs target)
- MoM delta
- QTD total vs QTD target
- Channel-specific activity metrics:
  - Inbound: SQLs, conversion-to-opp
  - Outbound: Accounts Engaged, Meetings Booked, Outbound SQOs
  - Product: PQLs, product-to-SQL conversion
  - Expansion: expansion opps created, CS-sourced expansion

Flag any channel that came in >20% below target as an Activity Shortfall risk.

### Forward-looking (included in Month-Ahead report)

Produce the Top 10 Accounts to Go After for the new month using:
1. Aero Fit Score (from Total Prospect Account Engaged tab)
2. Engagement recency (GTM Stage)
3. Opportunity size estimate
4. Timing signals (fiscal year end, funding event, hiring burst, etc.)

For each of the 10 accounts, include a 1–2 sentence WHY that references all 4 dimensions. Do not produce the list without WHYs — it is the WHYs that make the list actionable.

### Pipeline coverage math

At month-end:
- Open Pipeline $ / (Next-Month Bookings Target) = coverage ratio
- Flag if coverage < 3x as a Pipeline Gap risk (SDR Team owns)
- Flag by channel if any single channel is below 2x

Pipeline coverage is one of the most important forward-looking metrics in the monthly report. Narrate it with intent.
