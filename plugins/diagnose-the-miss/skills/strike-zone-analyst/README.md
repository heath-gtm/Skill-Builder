...
```bash
mkdir -p ~/.claude/skills/strike-zone-analyst && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/strike-zone-analyst/SKILL.md -o ~/.claude/skills/strike-zone-analyst/SKILL.md && echo "Installed strike-zone-analyst. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/strike-zone-analyst/`, then restart Claude Code.
