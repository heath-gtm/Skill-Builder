...
```bash
mkdir -p ~/.claude/skills/solve-the-problem && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/solve-the-problem/SKILL.md -o ~/.claude/skills/solve-the-problem/SKILL.md && echo "Installed solve-the-problem. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/solve-the-problem/`, then restart Claude Code.
