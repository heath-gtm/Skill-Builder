...
- LinkedIn publishes to **ORDINAL**, not Buffer. Buffer is retired and lib/buffer.ts is gone. The client is lib/ordinal.ts (createPost, schedulePost, addEngagements, linkedInPostAnalytics, linkedInFollowers). Ordinal requires publishAt on every post.
...
2. DRAFT. Moving a card to Draft fires the article process AUTOMATICALLY, whether Heath moves it, the calendar moves it, or Hermes or Claude moves it. It is idempotent and will not re-draft a card that already has a Ghost draft.
   ...
   - It then creates the Ghost draft, stores the Ghost post id, and generates THREE companion LinkedIn posts (scar same day, lens +2 days, receipt +6 days) attached to the anchor.
...
5. MEASURE. /api/cron/job/content-metrics pulls LinkedIn analytics from Ordinal back onto the cards, tagged by angle, so scar vs lens vs receipt is answered with data.
...
- PIECES: one anchor article per slot, plus companions with an angle (scar / lens / receipt) and a day offset.
...
LINKEDIN: SCAR, LENS, RECEIPT
Three posts per article, never three versions of one. Scar admits the failure (usually the strongest). Lens takes a position worth arguing with. Receipt hands over the artifact. Each must be a complete thought that opens a bigger question; a summary post kills the click.
