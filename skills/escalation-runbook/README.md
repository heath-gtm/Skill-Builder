...
```bash
mkdir -p ~/.claude/skills/escalation-runbook && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/escalation-runbook/SKILL.md -o ~/.claude/skills/escalation-runbook/SKILL.md && echo "Installed escalation-runbook. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/escalation-runbook/`, then restart Claude Code.
