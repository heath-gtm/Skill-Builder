...
```bash
mkdir -p ~/.claude/skills/refresh-orchestrator && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/refresh-orchestrator/SKILL.md -o ~/.claude/skills/refresh-orchestrator/SKILL.md && echo "Installed refresh-orchestrator. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/refresh-orchestrator/`, then restart Claude Code.
