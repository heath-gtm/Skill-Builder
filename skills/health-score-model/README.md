...
```bash
mkdir -p ~/.claude/skills/health-score-model && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/health-score-model/SKILL.md -o ~/.claude/skills/health-score-model/SKILL.md && echo "Installed health-score-model. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/health-score-model/`, then restart Claude Code.
