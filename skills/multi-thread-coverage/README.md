...
```bash
mkdir -p ~/.claude/skills/multi-thread-coverage && curl -fsSL https://raw.githubusercontent.com/heath-gtm/Skill-Builder/main/skills/multi-thread-coverage/SKILL.md -o ~/.claude/skills/multi-thread-coverage/SKILL.md && echo "Installed multi-thread-coverage. Restart Claude Code."
```

Or download `SKILL.md` from this folder and drop it into `~/.claude/skills/multi-thread-coverage/`, then restart Claude Code.
